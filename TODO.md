...
- [x] **OmniFocus backlink** — Added an "Add OmniFocus backlink" setting; once OmniFocus confirms a task was created, a `[🔗](omnifocus:///task/…)` link is inserted back into the Obsidian task line, via a registered `obsidian://` protocol handler and a hidden per-task marker (works across all three send modes; Plug-in mode requires reinstalling the companion plug-in).
